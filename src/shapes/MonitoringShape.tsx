...
export type MonitoringShape = TLBaseShape<
  'monitoring',
  {
    w: number
    h: number
...
export class MonitoringShapeUtil extends BaseBoxShapeUtil<MonitoringShape> {
  static override type = 'monitoring' as const

  static override props: RecordProps<MonitoringShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): MonitoringShape['props'] {
    return {
      w: 100,
      h: 80,
      color: 'black',
  ...
  override component(shape: MonitoringShape) {
  ...
  override indicator(shape: MonitoringShape) {
    const { w, h } = shape.props
