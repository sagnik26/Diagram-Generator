...
export type DnsShape = TLBaseShape<
  'dns',
  {
    w: number
    h: number
...
export class DnsShapeUtil extends BaseBoxShapeUtil<DnsShape> {
  static override type = 'dns' as const

  static override props: RecordProps<DnsShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): DnsShape['props'] {
    return {
      w: 100,
      h: 100,
      color: 'black',
  ...
  override component(shape: DnsShape) {
  ...
  override indicator(shape: DnsShape) {
    const { w, h } = shape.props
