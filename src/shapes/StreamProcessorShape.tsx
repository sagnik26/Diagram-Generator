...
export type StreamProcessorShape = TLBaseShape<
  'stream-processor',
  {
    w: number
    h: number
...
export class StreamProcessorShapeUtil extends BaseBoxShapeUtil<StreamProcessorShape> {
  static override type = 'stream-processor' as const

  static override props: RecordProps<StreamProcessorShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): StreamProcessorShape['props'] {
    return {
      w: 160,
      h: 80,
      color: 'black',
  ...
  override component(shape: StreamProcessorShape) {
  ...
  override indicator(shape: StreamProcessorShape) {
    const { w, h } = shape.props
