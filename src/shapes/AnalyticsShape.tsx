...
export type AnalyticsShape = TLBaseShape<
  'analytics',
  {
    w: number
    h: number
...
export class AnalyticsShapeUtil extends BaseBoxShapeUtil<AnalyticsShape> {
  static override type = 'analytics' as const

  static override props: RecordProps<AnalyticsShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): AnalyticsShape['props'] {
    return {
      w: 120,
      h: 80,
      color: 'black',
  ...
  override component(shape: AnalyticsShape) {
  ...
  override indicator(shape: AnalyticsShape) {
    const { w, h } = shape.props
