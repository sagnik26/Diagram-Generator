...
export type AlertingShape = TLBaseShape<
  'alerting',
  {
    w: number
    h: number
...
export class AlertingShapeUtil extends BaseBoxShapeUtil<AlertingShape> {
  static override type = 'alerting' as const

  static override props: RecordProps<AlertingShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): AlertingShape['props'] {
    return {
      w: 100,
      h: 80,
      color: 'black',
  ...
  override component(shape: AlertingShape) {
  ...
  override indicator(shape: AlertingShape) {
    const { w, h } = shape.props
