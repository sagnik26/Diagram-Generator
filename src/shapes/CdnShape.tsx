...
export class CdnShapeUtil extends BaseBoxShapeUtil<CdnShape> {
  static override type = 'cdn' as const

  static override props: RecordProps<CdnShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): CdnShape['props'] {
    return {
      w: 140,
      h: 100,
      color: 'black',
  ...
  override component(shape: CdnShape) {
  ...
  override indicator(shape: CdnShape) {
    const { w, h } = shape.props
    // Cloud-like rounded indicator
    return (
      <path
        d={`M ${w * 0.2} ${h * 0.3} Q 0 ${h * 0.2} 0 ${h * 0.5} Q 0 ${h * 0.8} ${w * 0.2} ${h * 0.7} L ${w * 0.8} ${h * 0.7} Q ${w} ${h * 0.8} ${w} ${h * 0.5} Q ${w} ${h * 0.2} ${w * 0.8} ${h * 0.3} Z`}
      />
    )
  }
