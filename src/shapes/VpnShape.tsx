...
export type VpnShape = TLBaseShape<
  'vpn',
  {
    w: number
    h: number
...
export class VpnShapeUtil extends BaseBoxShapeUtil<VpnShape> {
  static override type = 'vpn' as const

  static override props: RecordProps<VpnShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): VpnShape['props'] {
    return {
      w: 100,
      h: 100,
      color: 'black',
  ...
  override component(shape: VpnShape) {
  ...
  override indicator(shape: VpnShape) {
    const { w, h } = shape.props
