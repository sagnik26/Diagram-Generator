...
export class ApiGatewayShapeUtil extends BaseBoxShapeUtil<ApiGatewayShape> {
  static override type = 'api-gateway' as const

  static override props: RecordProps<ApiGatewayShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): ApiGatewayShape['props'] {
    return {
      w: 160,
      h: 80,
      color: 'black',
  ...
  override component(shape: ApiGatewayShape) {
  ...
  override indicator(shape: ApiGatewayShape) {
    const { w, h } = shape.props
    // Trapezoid indicator - top edge is narrower
    const topOffset = w * 0.2
    return (
      <path
        d={`M ${topOffset} 0 L ${w - topOffset} 0 L ${w} ${h} L 0 ${h} Z`}
      />
    )
  }
