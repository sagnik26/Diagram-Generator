...
export type MlModelShape = TLBaseShape<
  'ml-model',
  {
    w: number
    h: number
...
export class MlModelShapeUtil extends BaseBoxShapeUtil<MlModelShape> {
  static override type = 'ml-model' as const

  static override props: RecordProps<MlModelShape> = {
    w: T.number,
    h: T.number,
    color: T.string,
  ...
  override getDefaultProps(): MlModelShape['props'] {
    return {
      w: 140,
      h: 80,
      color: 'black',
  ...
  override component(shape: MlModelShape) {
  ...
  override indicator(shape: MlModelShape) {
    const { w, h } = shape.props
