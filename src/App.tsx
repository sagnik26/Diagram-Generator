...
import {
  BorderWidthProvider,
  useBorderWidthContext,
} from "./contexts/BorderWidthContext";
import CustomToolbar from "./components/CustomToolbar";
import TopNav from "./components/TopNav";
import ConnectionPoints from "./components/ConnectionPoints";
import TextEditor from "./components/tambo/TextEditor";
...
function setupTextEditing(editor: Editor) {
  ...
  editor.on("double-click" as any, (e: { point: { x: number; y: number } }) => {
    ...
    if (
      ...
      (hitShape.type === "geo" ||
        hitShape.type === "text" ||
        hitShape.type === "cylinder" ||
        hitShape.type === "hard-drive" ||
        hitShape.type === "search" ||
        hitShape.type === "box" ||
        hitShape.type === "microservice" ||
        hitShape.type === "server" ||
        hitShape.type === "api-gateway" ||
        hitShape.type === "load-balancer" ||
        hitShape.type === "authentication-service" ||
        hitShape.type === "notification-service" ||
        hitShape.type === "payment-gateway" ||
        hitShape.type === "message-queue" ||
        hitShape.type === "message-broker" ||
        hitShape.type === "stream-processor" ||
        hitShape.type === "event-bus" ||
        hitShape.type === "cdn" ||
        hitShape.type === "dns" ||
        hitShape.type === "firewall" ||
        hitShape.type === "vpn" ||
    ...
    rafId = requestAnimationFrame(() => {
      ...
      shapes.forEach((shape: TLShape) => {
        if (
          shape.type === "geo" ||
          shape.type === "cylinder" ||
          shape.type === "hard-drive" ||
          shape.type === "search" ||
          shape.type === "box" ||
          shape.type === "microservice" ||
          shape.type === "server" ||
          shape.type === "api-gateway" ||
          shape.type === "load-balancer" ||
          shape.type === "authentication-service" ||
          shape.type === "notification-service" ||
          shape.type === "payment-gateway"
        ) {
          // Store original border size if not already stored
          // Type assertion for shapes with size property
          const shapeWithSize = shape as TLShape & {
            props: { size?: "s" | "m" | "l" | "xl" };
          };
          if (!originalBorderSizes.has(shape.id)) {
            ...
            if (
              shapeWithSize.props.size !== originalSize &&
              Date.now() - lastToolbarUpdate > 500
            ) {
              editor.updateShape({
                id: shape.id,
                type: shape.type,
